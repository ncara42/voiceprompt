...
def mic_test_cmd(
    seconds: float = typer.Option(
        3.0, "--seconds", "-s", help="How long to sample the microphone for.",
    ),
) -> None:
    """Open the default mic and report peak/RMS levels.

    Use this when the macOS recording dot doesn't show up or when recordings
    come back silent. It bypasses the daemon, the hotkey, the visualizer,
    and the AI provider — just talks to PortAudio directly. If the macOS
    permission dialog has never been shown for the binary running this CLI,
    it should pop the first time you run this command.
    """
    import contextlib  # noqa: PLC0415
    import sys  # noqa: PLC0415
    import time  # noqa: PLC0415
    ...
    # Show available devices first so the user can confirm which one will be used.
    try:
        devices = rec_mod.list_input_devices()
    except Exception as e:  # noqa: BLE001
        console.print(f"[err]Could not enumerate input devices: {e}[/err]")
        raise typer.Exit(code=1) from None

    if not devices:
        console.print("[err]No input devices found. Plug in a mic or check Bluetooth.[/err]")
        raise typer.Exit(code=1)

    console.print("[label]Input devices:[/label]")
    for d in devices:
        marker = "›" if d["default"] else " "
        console.print(
            f"  {marker} [hint]{d['index']:>2}[/hint]  [value]{d['name']}[/value]  "
            f"[hint]({d['channels']}ch · {d['sample_rate']} Hz)[/hint]"
        )
    console.print()
    ...
    try:
        ...
        console.print(
            "[err]No default input device. Pick one in System Settings → Sound → Input.[/err]"
        )
        raise typer.Exit(code=2) from None
    except Exception as e:  # noqa: BLE001
        console.print(f"[err]Could not open mic: {e}[/err]")
        console.print(
            "[hint]On macOS this usually means Microphone permission is denied. "
            "Open System Settings → Privacy & Security → Microphone and enable "
            "the binary that runs voiceprompt.[/hint]"
        )
        raise typer.Exit(code=3) from None

    console.print(
        f"  [brand]Sampling[/brand] for [value]{seconds:.1f}s[/value] — "
        f"[hint]speak now…[/hint]"
    )
    ...
    try:
        while time.monotonic() < deadline:
            ...
            bar = "█" * min(20, int(peak / 1500))
            sys.stderr.write(
                f"\r  peak {peak:5d} {bar:<20}  "
            )
            sys.stderr.flush()
    except KeyboardInterrupt:
        pass
    ...
    console.print(f"  [label]Max peak observed:[/label] [value]{max_peak}[/value][hint]/32767[/hint]")

    if max_peak == 0:
        console.print(
            "\n[err]Peak was 0 — the mic stream opened but every sample was silent.[/err]"
        )
        console.print(
            "[hint]Most likely causes:\n"
            "  · Microphone permission denied for the binary running this command.\n"
            "    Open System Settings → Privacy & Security → Microphone and check\n"
            "    that the entry corresponding to voiceprompt is ENABLED.\n"
            "  · The selected input device is muted or disconnected (e.g. an\n"
            "    AirPods entry that isn't actually paired right now).\n"
            "  · Another app is using the mic exclusively (Zoom, Discord, …).[/hint]"
        )
        raise typer.Exit(code=4)
    if max_peak < 200:
        console.print(
            "\n[warn]Peak is very low.[/warn] [hint]Speak closer to the mic, "
            "or pick a different input in System Settings → Sound → Input.[/hint]"
        )
        return
    console.print(
        "\n[ok]Mic is working.[/ok] [hint]If the daemon still gets silence, restart it: "
        "voiceprompt stop && voiceprompt start.[/hint]"
    )
